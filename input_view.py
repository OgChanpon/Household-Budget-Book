...
class InputView(ctk.CTkFrame):
    ...
    def pick_date(self):
        ...
        def on_date_select():
            ...
            self.date_button.configure(text=self.selected_date.strftime("%Y-%m-%d"))
            self.popup_frame.destroy(); self.popup_frame = None
        ctk.CTkButton(self.popup_frame, text="決定", command=on_date_select, height=35).pack(pady=(0, 10), padx=10, fill="x")
    ...
    def save_data(self):
        ...
        if not amount: messagebox.showwarning("入力エラー", "金額は必須です。"); return
    ...
    def add_transaction(self, date_str, trans_type, amount, memo, payment_method):
        if not amount.isdigit() or int(amount) <= 0: messagebox.showerror("エラー", "金額は正の整数で入力してください。"); return
        try:
            conn = sqlite3.connect('kakeibo.db'); cursor = conn.cursor()
            cursor.execute('INSERT INTO transactions (date, type, amount, memo, payment_method) VALUES (?, ?, ?, ?, ?)',
                           (date_str, trans_type, int(amount), memo, payment_method))
            conn.commit(); conn.close()
            messagebox.showinfo("成功", "データを保存しました。"); self.clear_entries()
        except sqlite3.Error as e: messagebox.showerror("データベースエラー", f"データの保存に失敗しました。\n{e}")
